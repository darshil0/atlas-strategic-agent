...
import { Search, X, Import, Filter, Layers, Zap } from "lucide-react";
...
const TaskBank = ({ onAddTask, onClose }: TaskBankProps) => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return TASK_BANK.filter((task: BankTask) => {
      const matchesSearch = !query || task.description.toLowerCase().includes(query) || task.id.toLowerCase().includes(query);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/90 backdrop-blur-3xl border-l border-slate-800 shadow-3xl">
      <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/40">
        ...
        <button onClick={onClose} className="p-2 text-slate-500 hover:text-white bg-slate-800/50 rounded-lg transition-all">
          <X className="w-5 h-5" />
        </button>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={cn(
                ...
                !selectedTheme ? "bg-blue-600 border-blue-500 text-white" : "bg-slate-800 border-slate-700 text-slate-500 hover:text-slate-300"
              )}
            >
              All
            </button>
            {themes.map((t) => (
              <button
                ...
                className={cn(
                  "px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border transition-all",
                  selectedTheme === t ? "bg-slate-700 border-slate-600 text-white" : "bg-slate-800/40 border-slate-800 text-slate-500 hover:text-slate-300"
                )}
              >
                {t}
              </button>
            ...
            >
              <div className="flex justify-between items-center mb-2.5">
                <div className="flex gap-2">
                  <span className={cn("px-2 py-0.5 rounded-md text-[9px] font-black border", THEME_COLORS[task.theme] || "")}>
                    {task.theme}
                  </span>
                  <span className={cn("px-2 py-0.5 rounded-md text-[9px] font-black border", PRIORITY_COLORS[task.priority])}>
                    {task.priority}
                  </span>
                </div>
                <span className="text-[10px] font-mono text-slate-700">#{task.id}</span>
              </div>
              <p className="text-sm text-slate-300 font-medium leading-relaxed mb-4 group-hover:text-white transition-colors">{task.description}</p>
