...
const TaskCard: React.FC<TaskCardProps> = ({ task, isActive, isBlocked, onClick }) => {
  ...
  const getStatusColor = (status: TaskStatus) => {
    if (isBlocked && status === TaskStatus.PENDING) return 'border-slate-800 bg-slate-900 opacity-60';
    switch (status) {
    ...
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start">
              <p className={`text-sm font-semibold leading-tight ${task.status === TaskStatus.COMPLETED ? 'text-slate-500 line-through' : 'text-slate-200'}`}>
                {task.description}
              </p>
              <span className="text-[9px] font-mono text-slate-600 ml-2 uppercase shrink-0">#{task.id}</span>
            </div>
            ...
            <div className="mt-2 flex flex-wrap gap-2 items-center">
              ...
              {task.duration && (
                <span className="text-[9px] text-slate-500 font-mono">🕒 {task.duration}</span>
              )}
              {task.parentId && (
                <span className="text-[9px] text-slate-600 font-mono">Parent: #{task.parentId}</span>
              )}
            </div>
            ...
            <div className="mt-2 flex items-center justify-between">
               <span className="text-[8px] text-slate-600 uppercase tracking-widest font-black">{task.category || 'Strategic'}</span>
      ...
      {isExpanded && (
        <div className="border-t border-slate-800 bg-slate-950/50 p-3 animate-in fade-in slide-in-from-top-2">
          {task.output && (
            <div className="mb-3">
              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest block mb-1">Expected Output</span>
              <p className="text-[10px] text-slate-400 font-medium italic">{task.output}</p>
            </div>
          )}
          {task.result && (
            <div className="mb-3">
              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest block mb-1">Execution Log</span>
          ...
          {task.citations && task.citations.length > 0 && (
            <div className="pt-2 border-t border-slate-800/50">
              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest block mb-1">Grounding Citations</span>
              <div className="flex flex-wrap gap-2">
                {task.citations.map((c, i) => (
                  <a key={i} href={c.uri} target="_blank" rel="noreferrer" className="text-[10px] text-blue-400 hover:underline truncate max-w-[150px]">
                    🔗 {c.title || c.uri}
                  </a>
                ))}
              </div>
